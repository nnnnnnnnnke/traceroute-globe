...
- **崩れにくいルータ位置推定** — 各ホップに最大 3 つの位置候補を持ちます:
  逆引きホスト名に事業者自身が埋め込む地名コード (CLLI/IATA: `sttlwa`,
  `chcgil`, `lax` など、一部事業者固有の略号も)、[RIPE IPmap](https://ipmap.ripe.net/)
  (RIPE Atlas の遅延実測・IXP・geofeed・クラウドソース)、一般向け IP データ
  ベース (ip-api)。クライアントはホップ列全体を一括で解き (候補列に対する
  動的計画法)、光ファイバ中の光速 (≈ RTT 1ms あたり 100km) に対する違反
  (隣接ホップ間・発信元との距離) が最小になる組み合わせを、ソースの信頼順を
  タイブレークにして選びます。どの候補も説明できないホップは ⚠ 付きで
  地図から除外
...
固定位置どうしの区間は、海底ケーブルや道路には沿わせず、発光する弧で描きます。弧の上をパケットの光が、
区間ごとの片道時間 (RTT の差の半分) に比例した速さで順に流れます。
...
|---|---|
| `host` | 宛先を入れた状態で開く |
| `wait` | 1 ホップあたりの応答待ち (秒。既定 2、上限 5)。往復が 1 秒を超える経路では延ばさないと末尾が `*` になる |
| `cables=0` / `fiber=0` | 海底ケーブル / 陸上ファイバの層を最初から隠す |
| `ui=min` | 地球儀だけを表示する (ほかのページに埋め込むとき) |
| `run=1` | 初期化が済んだら自動でトレースする |

埋め込み元のページからは `postMessage({ type: "traceroute-globe:run" })` で打ち直せます。

追従カメラは、届いたホップを出発点から順に巡ります (まとめて届いても飛ばしません)。地球を一周する経路では、
巡り終わったあとパケットと同じ向きにゆっくり自転させ、全体を見せ続けます。地球儀をドラッグすると止まります。
...
| 発光する青緑のケーブル | 推定ケーブルのうち経路が実際に通る区間 (ホップ一覧の `🌊`)。経路線はその線形に沿う。候補名をクリックするとシステム全体が光る |
...
ケーブル推定 (`src/cables.ts`): 600km 以上かつ国境を跨ぐ区間を対象に (国内
区間は陸上扱い)、両端のホップからそれぞれ 350km 以内に線の端点 (着陸点) を
持ち、その 2 端点が区間を跨ぐだけ離れているケーブルを候補とし、着陸点までの
距離・区間長との差・線形長と実測 RTT 増分の整合で順位付けします。経路線を
ケーブル線形に沿わせるのは、分岐を辿った最短路が直線の 1.6 倍以内のときだけ
(それ以上はパケットの経路ではない迂回線になるため)。
